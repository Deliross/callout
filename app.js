...
function adminControlView() {
  ...
  const waitingFeatures = `<section class="waiting-features admin-console-block"><header><div><h2>Waiting Features</h2><p>Hidden features keep their code and records.</p></div><span class="admin-lock">OWNER ONLY</span></header><div class="waiting-feature-list">${[['battles','Battles'],['heat-wheel','Heat Wheel'],['take-rush','Take Rush'],['swipe','Swipe'],['loops','Loops']].map(([key,name]) => { const enabled = controls.features.find(f => f.key === key)?.enabled ?? Boolean(state.features[key]); return `<article class="${enabled ? 'is-visible' : 'is-waiting'}"><div><h3>${name}</h3><p>${key === 'battles' ? 'Community tournaments. Records are preserved.' : ['swipe','loops'].includes(key) ? 'Parked for a future redesign. Existing posts and code are preserved.' : 'Practice prototype only. No real Heat is awarded.'}</p><span class="status-badge">${enabled ? 'VISIBLE' : 'WAITING'}</span></div><button type="button" class="primary-action" data-waiting-feature="${key}" data-next-enabled="${!enabled}">${enabled ? 'Hide from site' : 'Show on site'}</button><button type="button" class="secondary-action" data-preview-feature="${key}">Preview ${name}</button></article>`; }).join('')}</div></section>`;
...
function renderRoute() {
  ...
  const featureNames = { battles: 'Battles', 'heat-wheel': 'Heat Wheel', 'take-rush': 'Take Rush', swipe: 'Swipe', loops: 'Loops' };
  ...
  if (!featureBlocked && (route === 'swipe' || route === 'loops')) CalloutDiscovery.mount(route, mainContent, {
...
function updateComposerPreview() {
  const loopAvailable = Boolean(state.features?.loops || (sessionUser?.isAdmin && state.featurePreview === 'loops'));
  const loopOption = document.querySelector('#postFormat option[value="loop"]');
  loopOption.hidden = !loopAvailable; loopOption.disabled = !loopAvailable;
  if (!loopAvailable) document.querySelector('#postFormat').value = 'normal';
