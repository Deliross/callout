...
const userSchema = new mongoose.Schema({
  ...
  automationPersona: { type: String, default: '', maxlength: 80 },
  ...
  heatScore: { type: Number, default: 0, min: 0, index: true },
  heatStreakCurrent: { type: Number, default: 0, min: 0 },
  heatStreakLongest: { type: Number, default: 0, min: 0 },
  heatLastActiveDate: { type: String, default: '', match: /^(?:|\d{4}-\d{2}-\d{2})$/ },
  heatActivityDates: [{ type: String, match: /^\d{4}-\d{2}-\d{2}$/ }],
  ...
  profileLayout: [{ type: String, enum: ['posts', 'guilds', 'heat'] }],
  ...
  staffRole: { type: String, enum: ['member', 'moderator', 'admin', 'owner'], default: 'member', index: true },
